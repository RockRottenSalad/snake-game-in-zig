...
pub fn build(b: *std.Build) !void {
    ...
    if (target.getOsTag() == .emscripten) {
        const exe_lib = rl.compileForEmscripten(b, "test", "src/main.zig", target, optimize);
    ...
    const exe = b.addExecutable(.{ .name = "test", .root_source_file = .{ .path = "src/main.zig" }, .optimize = optimize, .target = target });
